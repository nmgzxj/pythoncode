...
def printHelp():
    print u"[q]:退出 [h]:帮助 [F]:显示所有板块 [F {id} {page}]:显示指定板块的指定页 [t {id} {page}]:显示指定串指定页 [f]:前进历史记录 [b]:后退历史记录 [p]:前一页 [n]:后一页 [T]:切换为有/无图模式（屏蔽有图Po主串) [c]艹！老板来了(伪装输出在同目录《theFile》内"
    print u"当前为" + (u"有" if picAvailable else u"无") + u"图模式"
